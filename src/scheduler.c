...
extern bool all_players_done(GameState *gs);
...
void* scheduler_thread_func(void* arg) {
    ...
    while (1) {
        ...
        if (need_pass_turn) {
            extern int find_next_player(GameState *gs, int current);
            int next = find_next_player(gs, current);
            
            if (next != -1) {
                gs->current_turn = next;
                gs->players[next].last_active = time(NULL);
                printf("[SCHEDULER] Turn passed to Player %d\n", next);
            } else {
                if (all_players_done(gs)) {
                    printf("[SCHEDULER] All players finished! Determining winner...\n");
                    determine_winner(gs);
                }
            }
        }

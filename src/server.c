...
extern void init_score_system();
extern void shutdown_score_system();
...
void handle_signal(int sig) {
    ...
    if (gs != NULL) {
        shutdown_score_system();
        cleanup_shared_memory(gs);
    }
    exit(0);
}
...
int main() {
    ...
    // Initialize Logger
    init_logger();
    log_event("SYSTEM", "Blackjack Server starting...");

    // Initialize Shared Memory
    gs = setup_shared_memory();
    if (!gs) {
        log_event("ERROR", "Failed to setup shared memory");
        exit(1);
    }
    ...
    // Initialize Score System (now that gs is ready)
    init_score_system();
    ...
    while (1) {
        ...
        extern void log_player_connect(int id);
        log_player_connect(my_id);
